	...
	def confidence(self):
		...
		for s in range(self.nS):
			for a in range(self.nA):
                ...
                # Reward confidence
				log_term_R = np.log((2 * self.nS + self.nA * self.t_k) / self.delta)
				self.confR[s, a] = np.sqrt((3.5 / n) * log_term_R)
				
                # Transition confidence
				log_term_P = np.log((2 * self.nA * self.t_k) / self.delta)
				self.confP[s, a] = np.sqrt((14 * self.nS / n) * log_term_P)
...
def run():
	...
	UCRL2_ = UCRL2(nS, 2, epsilon, delta)
	...
	for i in range(nb_Replicates):
		...
		env.reset()
		UCRL2_.reset(env.s)	# here the episode counter should reset as well!
		reward = 0
		new_s = env.s
		for t in range(T):
			action, _, _ = UCRL2_.play(new_s, reward)
		...
		episode_counts_list.append(UCRL2_.episode_count)
	...
	plot([cumregret_UCRL2], ["UCRL2"], y_label = "Cumulative Regret", exp_name = "cumulative_regret", delta=delta)

	plot([empirical_gain_data], ["UCRL2_empirical_gain"], y_label = "Empirical Gain", exp_name = "empirical_gain", delta=delta, gstar=gstar)
